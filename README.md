...
Сравнение Naive vs Brandes в `Run` сначала делается по точным рациональным массивам (`num/den`).
Если на больших графах возникает расхождение из-за роста рациональных дробей, включается резервная проверка `approximatelyEqual` по `real`-версиям алгоритмов (с `eps=1e-9`).
